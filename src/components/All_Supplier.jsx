...
function Allsupplier() {
  ...
  useEffect(() => {
    // Fetch supplier data from your API endpoint
    fetch(`${apiBaseUrl}/supplier/readAllSupplier`) // Replace with your actual API endpoint
      ...
      <table className="table">
        <thead>
          <tr>
            ...
            <th className="bg-success text-white text-center">Due</th>
            <th
...
onClick={() => {
  ...
  customerData[selectedRowIndex].purchase
    .slice()
    .reverse()
    .forEach((product, productIndex) => {
      printWindow.document.write(
        `<tr><td>${productIndex + 1}</td><td>${product.purchaseDate}</td><td>${product.quantity} QTL.</td><td>${product.productName}</td><td>₹${product.amount}</td></tr>`
      );
    });
